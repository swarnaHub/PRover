...
class RobertaForRRWithEdgeLoss(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, proof_offset=None,
        ...
        outputs = (logits, edge_logits) + outputs[2:]
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            qa_loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
        ...
        return outputs  # (total_loss), qa_loss, edge_loss, logits, edge_logits, (hidden_states), (attentions)
