...
def solve_LP(edge_logits, fact_rule_identifier, node_labels):
    ...
    all_flow_vars[(source_id, node_ids_present[0])] = LpVariable("Flow_source_" + str(node_ids_present[0] + 1) , 0, 100, LpInteger)
    ...
    prob += all_flow_vars[(source_id,node_ids_present[0])] <= C[(source_id,node_ids_present[0])] , "Capacity constraint source "+str(node_ids_present[0])
    ...
    for n in range(len(all_edge_logits)):
    ...
    for i in range(len(edge_logits)):
        for j in range(len(edge_logits)):
            ...
            prob += (all_vars[i,j,1] + all_vars[j,i,1] ) - (all_flow_vars[(i,j)] / C[(source_id, node_ids_present[0])]) >= 0 , "Valid flow "+str(i+1)+" "+str(j+1)
    '''
...
if __name__ == '__main__':
    edge_logit_file = open("../output/cycle_model_naf_layer_classification_head_edge_masking_ep_5/prediction_edge_logits_dev.lst", "r", encoding="utf-8-sig")
    edge_logits = edge_logit_file.read().splitlines()

    node_pred_file = open("../output/cycle_model_naf_layer_classification_head_edge_masking_ep_5/prediction_nodes_dev.lst", "r", encoding="utf-8-sig")
